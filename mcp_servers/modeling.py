"""AutoML MCP — Parallel AutoML Engine (in-process)."""
...
async def run_parallel_bake_off(
    ...
    Champion pipeline + X_train sample are saved as local artifacts.
    ...
    model_art = await asyncio.to_thread(
        save_artifact,
    ...
    xtrain_art = await asyncio.to_thread(
        save_artifact,
        local_path=sample_path, kind="xtrain_sample",
        metadata={"linked_model_id": model_art.id},
    )
    ...
    return {
        ...
        "model_id": model_art.id,            # << use this for SHAP
        "x_train_sample_id": xtrain_art.id,  # << and this
